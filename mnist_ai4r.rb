...
nn = Ai4r::NeuralNetwork::Backpropagation.new [28*28,100,10]
...
100.times do |i|
  errsum = 0
  train_data.each do |input, expected|
    errsum += nn.train(input, expected)
  end
  avg_error = errsum / train_data.length.to_f
  puts "error at #{i + 1}: #{avg_error}"
  break if avg_error < 0.01
end
puts "\nDone training the network. #{(Time.now - t).round(1)}s"
