...
train_inputs = train_data.map {|row| row[0]}
train_outputs = train_data.map {|row| row[1]}

train = RubyFann::TrainData.new(:inputs=>train_inputs, :desired_outputs=> train_outputs)

# nn = NeuralNet.new [28*28,100,10]
fann = RubyFann::Standard.new(:num_inputs=>28*28, :hidden_neurons=>[100], :num_outputs=>10)
...
success, failure, avg_mse = run_test.(fann, test_data)
